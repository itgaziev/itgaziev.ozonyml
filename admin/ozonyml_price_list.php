...
if($lAdmin->EditAction() && $POST_RIGHT == 'W') {
    foreach($FIELDS as $ID => $arFields) {
        ...
        if($ID > 0) {
            ...
            $result = OzonYML\Table\ITGazievOzonYMLTable::update($ID, $arData);

            if(!$result->isSuccess())
                $lAdmin->AddGroupError(Loc::getMessage('ITGAZIEV_OZONYML_PRICE_SAVE_ERROR'), $ID);
        } else {
            $lAdmin->AddGroupError(Loc::getMessage('ITGAZIEV_OZONYML_PRICE_SAVE_ERROR'), $ID);
        }
    }
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT == 'W') {
    if($_REQUEST['action_target'] == 'selected') {
        $rsData = OzonYML\Table\ITGazievOzonYMLTable::getList(array(
    ...
    foreach($arID as $ID) {
        ...
        switch($_REQUEST['action']) {
            case 'delete':
                $result = OzonYML\Table\ITGazievOzonYMLTable::delete($ID);
                if(!$result->isSuccess()) $lAdmin->AddGroupError(Loc::getMessage('ITGAZIEV_OZONYML_PRICE_DELETE_ERROR'), $ID);
...
$lAdmin->AddHeaders(array(
    ...
    array(
        'id'        => 'NAME',
        'content'   => Loc::getMessage('ITGAZIEV_OZONYML_PRICE_TABLE_NAME'),
        'sort'      => 'NAME',
        'default'   => true,
    ),
    array(
        'id'        => 'TIME_CREATE',
        'content'   => Loc::getMessage('ITGAZIEV_OZONYML_PRICE_TABLE_TIME_CREATE'),
        'sort'      => 'TIME_CREATE',
        'default'   => true
    ),
    array(
        'id'        => 'RUN_PROCESS',
        'content'   => Loc::getMessage('ITGAZIEV_OZONYML_PRICE_TABLE_RUN_PROCESS'),
        'sort'      => '',
        'default'   => true
    )
));
...
while($arRes = $rsData->NavNext(true, 'f_')) {
    ...
    $arActions[] = array(
        'ICON' => 'edit',
        'DEFAULT' => true,
        'TEXT' => Loc::getMessage('ITGAZIEV_OZONYML_PRICE_EDIT_BTN'),
        'ACTION' => $lAdmin->ActionRedirect('itgaziev.ozonyml_price_edit.php?ID=' . $f_ID . '&lang=' . LANG)
    );
...
$lAdmin->AddFooter(array(
    array('title' => Loc::getMessage('ITGAZIEV_OZONYML_PRICE_LIST_SELECTED'), 'value' => $rsData->SelectedRowsCount()),
    array('counter' => true, 'title' => Loc::getMessage('ITGAZIEV_OZONYML_PRICE_LIST_CHECKED'), 'value' => '0')
));
...
$aContext = array(
    array(
        'TEXT' => Loc::getMessage('ITGAZIEV_OZONYML_PRICE_ADD_TEXT'),
...
$APPLICATION->SetTitle(Loc::getMessage('ITGAZIEV_OZONYML_PRICE_LIST_TITLE'));
...
$oFilter = new CAdminFilter(
    ...
    array(
        'ID',
        Loc::getMessage("ITGAZIEV_OZONYML_PRICE_FILTER_FIND_NAME")
    )
);
